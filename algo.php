...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"),
            "SELECT * FROM user_teacher WHERE faculty_number='$temp->subjectteacher'");
...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"), "SELECT * FROM user_teacher WHERE deleted=0 ");
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = ($sem + 1) / 2;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"),
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"),
        "SELECT * FROM user_teacher WHERE faculty_number='$temp->subjectteacher' AND deleted=0");
    $row = mysqli_fetch_assoc($teacheralias_query);
    $temp->alias = $row['alias'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT * FROM user_teacher WHERE faculty_number='$temp->subjectteacher2'AND deleted=0");
    $row = mysqli_fetch_assoc($teacheralias_query);
    $temp->alias2 = $row['alias'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT * FROM user_teacher WHERE faculty_number='$temp->subjectteacher3'AND deleted=0");
    $row = mysqli_fetch_assoc($teacheralias_query);
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "timetable"), $query);
    }
